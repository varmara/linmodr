...
Norm <- data.frame(x, y)
ggplot(data = Norm, aes(x=x, y=y)) + geom_line(color="steelblue", size=2) + geom_vline (xintercept = Mu) + labs(title = "Normal distribution", x = "Values", y = "Probability")
```
...
```{r echo=FALSE, purl=FALSE, warning=FALSE}
gg_means <- ggplot(data = data.frame(means = my_means), aes(x = means)) + geom_histogram(binwidth=1, fill="steelblue", color="black") + labs(title = "100 sample means, N = 5") + xlim(30, 70) + geom_vline(xintercept = Mu, colour = "red", size = 1) + geom_vline(xintercept = mean(my_means), colour = "red", size = 1, linetype = "dashed")
gg_means
```
...
gg_means_30 <- ggplot(data = data.frame(means = my_means_30), aes(x = means)) + geom_histogram(binwidth=1, fill="steelblue", color="black") + labs(title = "100 sample means, N = 30") + xlim(30, 70) + geom_vline(xintercept = Mu, colour = "red", size = 1) + geom_vline(xintercept = mean(my_means_30), colour = "red", size = 1, linetype = "dashed")
...
t <- seq(-10,10,0.1)
pt <- dt(t, 4)
ggplot(data.frame(t=t, pt=pt), aes(x=t, y=pt)) + geom_line(size=2) + geom_vline(xintercept = 0) + xlab("Standardized difference between means") + ylab("Probability")
```
...
gg_t_our <- ggplot(t_dist, aes(x = t)) + 
  ...
  geom_vline(xintercept = -2, linetype = 5, size = 1) + 
  geom_vline(xintercept = 2, linetype = 5, size = 1) + 
  labs(title = "Manually constructed t-distribution") + 
  xlim(-5, 5) + ylim(0, 60) + 
  geom_histogram(x = t_dist$t_exeeded,
...
```{r echo=FALSE, purl=FALSE}
t_df <- data.frame(t = seq(-5, 5, length.out = 100))

dt_limit <- function(x, alph = 0.05, df = df, sides = 2) {
...
gg_t <- ggplot(data = t_df, aes(x = t)) +
  stat_function(fun=dt, args=list(df=18), size = 1, geom="line", colour="steelblue") + 
...
gg_t + 
  stat_function(fun=dt_limit1, args=list(alph = 0.025, df=18), geom="area", fill="red", alpha = 0.5) + 
  stat_function(fun=dt, args=list(df=18), geom="line", colour="steelblue") + 
  geom_text(x=-4, y=0.1, label= "Редкие значения") + 
  geom_text(x=4, y=0.1, label= "Редкие значения") + 
  geom_text(x=0, y=0.1, label= "Обычные значения") + 
...
> - Теперь у нас есть инструмент для проверки статистических гипотез - __статистический критерий__, или __статистический тест__
...
dperm_pl <- ggplot(df_perm, aes(x = d_p, fill = less)) + 
  ...
  geom_vline(xintercept = d_initial, linetype = 2) +
...
sum(dperm >= d_initial) / Nperm
(p_perm <- mean(dperm >= d_initial))
```
