...
- $E(y)  = \mu$ -- мат.ожидание
- $var(y) = 1$ -- функция дисперсии (variance function)
- $-\infty \le y \le +\infty$,  $y \in \mathbb R$ -- диапазон значений
...
- $E(y) = \mu$ -- мат.ожидание
- $var(y) = \frac {\mu^2}{\nu}$ -- функция дисперсии
- $0 < y \le +\infty$, $y \in \mathbb R$ -- диапазон значений
...
- $var(y) = n \cdot \pi \cdot (1-\pi)$ -- функция дисперсии
- $0 \le y \le +\infty$, $y \in \mathbb{N}$ -- диапазон значений
...
Функция связи $g(\mu)$ используется разная в зависимости от распределения $f(y_i|\theta)$. Например:
...
Нужно найти параметры  $\theta$, которые максимизирут правдоподобие: $L(\theta | \text{data}) \longrightarrow \text{max}$

Вычислительно проще работать с логарифмами правдоподобий (loglikelihood): $lnL(\theta| \text{data}) \longrightarrow \text{max}$
...
$\begin{aligned}\eta_i &= 38.12 - 0.24 Age_{i} + 1.28 Month_{Nov\,i} + \\ &+ 0.02 Age_{i} Month_{Nov\,i}\end{aligned}$
...
Модель объясняет `r round((mod$null.deviance - mod$deviance) / mod$null.deviance *100, 1)`\% общей девиансы.
