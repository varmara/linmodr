...
# ЧАСТЬ 2. Cвязка медиана + квартили; 5-number summary.
...
Квантили --- это значения, которые делят распределение на равные части.
...
Персентиль --- это частный случай квантиля. Всего 100 персентилей.
...
Расстояние между I и III квартилями (высота "коробочки") называется _интерквартильное расстояние_
...
## Квартили и боксплот: непараметрические характеристики выборки
...
Нормальных кривых бесконечно много, и их описывает заковыристая формула с двумя параметрами ($\mu$ и $\sigma$).

$$p= \frac {1}{\sigma \sqrt{2 \pi}} e^{-\frac{(x-\mu)^2}{2\sigma^2}}$$

Достаточно знать эти два значения чтобы восстановить или смоделировать любое нормальное распределение.
...
### Дисперсия --- $s^2$ (выборочная оценка параметра $\sigma^2$)

$$s^2=\frac{\sum{(x_i - \bar{x})^2}}{n-1}$$

### Стандартное отклонение --- $s$ или SD (выборочная оценка параметра $\sigma$)
...
```{r results="hide"}
sqrt(sum(raw.deviates^2) / (length(apples) - 1))
```
...
При расчете стандартного отклонения R автоматически вносит поправку и использует степени свободы, а не "сырой" объем выборки.
