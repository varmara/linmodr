...
pl_tot <- ggplot(brain, aes(x = MRINACount, y = PIQ)) + 
  ...
  geom_segment(aes(x = MRINACount, y = PIQ, 
                   xend = MRINACount, yend = gmean), colour = "grey50") + 
  geom_point() +
...
pl_all <- ggplot(brain, aes(x = MRINACount, y = PIQ)) +  
  ...
  geom_hline(yintercept = gmean, size = 2) + 
      annotate("text", label = "Общее среднее", 
             x = 1050000, y = gmean - 8, size = 4, hjust = 0.7) + 
  annotate("segment", x = X - 1500, y = Y, xend = X - 1500, yend = gmean, colour = "grey50", size = 2) + 
  annotate("segment", x = X + 2000, y = Y, xend = X + 2000, yend = Y_hat, colour = "#E69F00", size = 2) +
  annotate("segment", x = X + 2000, y = Y_hat, xend = X + 2000, yend = gmean, colour = "#009E73", size = 2) +
  geom_point(size = 2)
...
pl_no <- ggplot(brain, aes(x = MRINACount, y = PIQ)) +  
  ...
  annotate("segment", x = X - 1500, y = Y, xend = X - 1500, yend = gmean, colour = "grey50", size = 2) + 
  annotate("segment", x = X + 2000, y = Y, xend = X + 2000, yend = gmean, colour = "#E69F00", size = 2) +
  geom_point(size = 2)
...
В случае, если между $x$ и $y$ нет зависимости, линия регрессии должна пройти параллельно оси $х$ на уровне среднего значения $y$. Значит отклонения наблюдений от общего среднего должны быть равны отклонениями от предсказанных значений.
...
ggplot(data.frame(f = seq(-0.3, 8, 0.00001)), aes(x = f)) + 
  ...
  ggtitle("F-распределение, df1 = 1, df2 = 38") + xlab("F")
```
