...
Даже ничего не считая специально, мы примерно понимаем, какой сорт молока дороже, какой дешевле.
...
Предположим, мы занимаемся селекцией яблонь и хотим охарактеризовать урожай нашей любимой яблони, на которую мы возлагаем большие надежды.
...
Как видим, оно не так благополучно пережило это вторжение. Единственное неверное измерение сместило среднее значение из разряда довольно крупных яблок в область гигантских грейпфрутов.
...
```{r boxplot.plain.1, fig.height = 6, fig.width = 4, purl=FALSE}
boxplot(apples)
```
...
В файле `diatome_count.csv` даны количества диатомовых водорослей в желудках этих животных. Прочитаем эти данные и посмотрим на них:
...
Так что же это за чудодейственные параметры? $\mu$ --- среднее, $\sigma$ --- стандартное отклонение.
...
$n - 1$ --- это число независимых значений (число степеней свободы).
