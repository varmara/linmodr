...
- $\begin{array}{l}\eta_{ij} = \beta_0 + \beta_{1}SexParent_{M} + \beta_{2}FoodTreatment_{S} + \beta_{3}ArrivalTime + \\ + \beta_{4} SexParent_{M}:FoodTreatment_{S} + \beta_{5}SexParent{M}:ArrivalTime + log(BroodSize) + a_{i} \end{array}$
...
- $E(NCalls_{ij}) = \mu_{ij}$, $var(NCalls_{ij}) = \mu_{ij} + \mu^2_{ij} / k$
- $ln(\mu_{ij}) = \eta_{ij}$  --- функция связи --- логарифм
- $\eta_{ij} = \beta_0 + \beta_{1}SexParent_{M} + \beta_{2}FoodTreatment_{S} + \beta_{3}ArrivalTime + log(BroodSize) + a_{i}$
