...
>- $G^2$ - подчиняется $\chi^2$ распределению (с параметром df = 1) если нулевая модель и предложенная модель не отличаются друг от друга.
...
ggplot(M15_diag, aes(x = .fitted, y = .pears_resid)) + 
...
<!-- ggplot(M15_diag, aes(x = .pears_resid)) + geom_histogram(fill = "blue", color = "black", binwidth = 0.2) + geom_vline(aes(xintercept = 0), size = 1.2) -->
<!-- ``` -->
...
<!-- Predicted <-predict(M15, newdata = MyData,  -->
<!--                     type ="response")  -->
