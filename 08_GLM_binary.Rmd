...
# Кратко вспомним о методе макcимального правдоподобия
...
- При разных значениях предикторов вероятности событий подчиняются разным распределениям (not identically distributed)
...
```{r}
pear_res <- resid(liz_model, type = "pearson") #Пирсоновские остатки
p <- length(coef(liz_model)) #число параметров в модели  
N <- nrow(model.frame(liz_model)) # объем выборки
...
Как поступать в случае присутствия избфточности дисперсии будет отдельный разговор.
