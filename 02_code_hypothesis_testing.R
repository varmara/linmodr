...
# Давайте разберемся на примере этого неправильного графика, как устроены графики пакета `ggplot`
...
gg_height <- ggplot(df_height, aes(x = height, fill = gender)) +
  geom_histogram(binwidth = 3, colour = "grey40", position = "dodge") +
  geom_vline(x = mean_f, colour = "red", size = 1) +
  geom_vline(x = mean_m, colour = "blue", size = 1, linetype = "dashed")
gg_height

## Добавим подписи осей и заголовок
### Так выглядит частотное распределение этих двух выборок
gg_height +
  labs(x = "Height (cm)",
       y = "Count",
...
# В него надо вписать исходное, полученное до пермутаций, значение d_initial
...
### Постройте график распределения d-статистики самостоятельно
