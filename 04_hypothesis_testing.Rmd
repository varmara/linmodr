...
Осторожно! Группы могут быть неоднородны. Здесь неучтен размер и т.п.
...
### Шаг 2. Проверяем __условия применимости__ тестовой статистики.

### Шаг 3. По реальным данным вычисляем __эмпирическое значение тестовой статистики__.
...
### Шаг 4. Строим теоретическое распределение тестовой статистики для случая, когда верна $H_0$, и оцениваем по нему уровень значимости.

### Шаг 5. Решаем сохранить или отвергнуть $H_0$.
...
<!-- Будем считать, что можно апроксимировать концентрацию тетрагидрокортизона нормальным распределением. -->
...
($t_{`r round(tt$parameter, 2)`} = `r round(tt$statistic, 2)`$, $p = `r format.pval(tt$p.value, eps = 0.001)`$).

## Двухвыборочный t-тест в R: способ 2.
...
Хорошо. Можно апроксимировать нормальным распределением 
