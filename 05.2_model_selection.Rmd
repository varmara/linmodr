...
>- Для этих данных можно подобрать несколько моделей. Попробуем посмотреть, как это будет выглядеть на примере loess--- локальной полиномиальной регрессии. (Если интересно, [подробнее о loess-регрессии](https://www.mathworks.com/help/curvefit/smoothing-data.html#bq_6ys3-4))
...
- Разные способы подбора оптимальной модели могут приводить к разным результатам. Не важно, какой из способов выбрать, но важно сделать это заранее, __до анализа__, чтобы не поддаваться соблазну подгонять результаты.

- При выборе моделей на основании F-тестов или тестов отношения правдоподобий приходится делать множество тестов, поэтому увеличивается риск ошибок I рода (подробнее на занятии про дисперсионный анализ). Чтобы хоть как-то себя обезопасить, лучше не включать в модель все подряд --- только самое необходимое.
