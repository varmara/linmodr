...
Эта прямая описывает связь между  $Wt$ и $Init$ для той градации фактора `Treatment`, которая взята за **базовый уровень**. По умолчанию в качестве базового уровня берется первая по алфавиту градация дискретного фактора. В данном случае - это градация `intensive`
